public class ListaDoble{
	...
	public Node buscarNodo(String buscado) {
		...
		while (!temp.name.equals(buscado) == true) {
			temp = temp.next;
		}
	...
	public void insertarDespuesDeNodo (String dato, String buscado)
    {

        Node temp1 = buscarNodo(buscado);
    ...
    public void cambioDeNodo (String dato, String buscado){
    	Node temp = buscarNodo(buscado);

        if (temp != null)
        {
            Node temp2 = topForward;

            while (!temp2.name.equalsIgnoreCase(buscado))
            {
                temp2 = temp2.next;
            }
